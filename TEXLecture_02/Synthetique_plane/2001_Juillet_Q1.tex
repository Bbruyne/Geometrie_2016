		...
		\column{.52\textwidth}\centering 
			...
			\underline{Dessin}\\
				  ...
				  \coordinate (A') at (0:2);
			          ...
			          \draw[visible on=<{1,2,5}>] (O) node[left] {$O$} -- (A');
			          \draw[visible on=<{1,2,5}>] (A') node [right]{$A'$};
			          \draw[visible on=<{1,3,5}>] (O) -- (B');
			          \draw[visible on=<{1,3,5}>] (B') node [below left]{$B'$};
			          \draw[visible on=<{1,4,5}>] (O) -- (C');	
			          \draw[visible on=<{1,4,5}>] (C') node [above]{$C'$};
			          \draw[visible on=<{1,2,5}>] (A) -- (A');
			          \draw[visible on=<{1,3,5}>] (B) -- (B');
			          \draw[visible on=<{1,4,5}>] (C) -- (C');			         
