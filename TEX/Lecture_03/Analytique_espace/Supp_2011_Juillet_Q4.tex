	  ...
	  \medskip
		 ...
		 \column{.20\textwidth}\centering       
	      ...
	      \end{columns}
				    \end{tcolorbox}
		\centering 
		$\mathcal{D}(d_1,\Pi)=\mathcal{D}(A,\Pi)\quad \text{ car $d_1\parallel\Pi$}$ \\ \vspace{8mm}
		\begin{columns}[c]	
		\column{.5\textwidth}\centering
		\begin{figure}[h]
		...
		$$\mathcal{D}(A,\Pi) = \frac{|n_{\Pi_x}A_x + n_{\Pi_y}A_y + n_{\Pi_z}A_z|}{|\vect{n_{\Pi}}|}$$
		\end{columns} \vspace{8mm}
		De (a), $\ \Pi \equiv x-2y+2z-12=0 \quad \rightarrow \quad \vect{n_{\Pi}}=(1,-2,2)$. \\ \bigskip
