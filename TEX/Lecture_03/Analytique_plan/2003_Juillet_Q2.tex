...
\tikzset{
  ...
  \def\enonce{  \frametitle{Q2 Juillet 2003.}
...
\begin{document}  
    ...
    \frame{
	  ...
	  \pause
	   ...
	   \begin{tcolorbox}[basic] \smallskip
	     ...
	     Dans un repère, établir les coordonnées de $D$ pour trouver l'équation cartésienne du lieu.
    ...
    \frame{\begin{columns}[t]
		...
		\column{.52\textwidth}\centering 
		 ...
		 \textit{Cas particulier : $c=0$} \\ \medskip
                  $AC \equiv \begin{cases}A,\\C.\end{cases}\rightarrow
		 ...
		 $G \equiv \begin{cases}
		            x=\dfrac{b}{3}, \\[0.8em]
		            y=\dfrac{a+y_D}{3}.
		           \end{cases} \quad (y_D \in \mathbb{R})$ \\ \medskip
		 Lieu : $x=\frac{b}{3}$.
    ...
    \frame{ \begin{columns}[t]
		...
		\column{.55\textwidth} \underline{Résolution}\\ \flushleft
		
		 
                \textit{Cas général : $c\neq 0$} \\ \medskip
                   $AC \equiv \begin{cases}A,\\C.\end{cases}\rightarrow
		 AC \equiv y=a\left(1-\dfrac{x}{c}\right).$ \\ \medskip
		 $\rightarrow D(\ x_D,\ a(1-\frac{x_D}{c})\ ). \qquad (x_D \in \mathbb{R})$ \\ \bigskip
		 $G = \frac{1}{3}A+\frac{1}{3}B+\frac{1}{3}D$. \\ \medskip
		 \begin{numcases}{G \equiv} 
		            x=\frac{b+x_D}{3}, \\[0.2em]
		            y=\frac{a+a\left(1-\dfrac{x_D}{c}\right)}{3}.
		           \end{numcases} \\ \medskip
		 $(1)\rightarrow \ x_D = 3x -b$.\hfill $(3)$ \\ \bigskip
		 $(1),(3)\rightarrow  \text{Lieu : } y=-\dfrac{a}{c}x + \dfrac{a}{3c}(b+2c)$.\\ \medskip \hfill
                 $(x \in \mathbb{R})$\\ \hfill $\qed$
	...
	.7 L'équation du lieu est une droite car $x\in \mathbb{R}$..
	  ...
	  \underline{Résumé}\\ \flushleft
          \begin{itemize}
          \item Trouver les cas particuliers. Par exemple en vérifiant qu'aucun dénominateur ne peut s'annuler.
          \item Si $G$ est le centre de gravité de $A,B,C$, alors

            $$G=\left(\dfrac{A_x + B_x + C_x}{3}, \dfrac{A_y + B_y + C_y}{3}\right).$$.
          \end{itemize}
    }
