...
\tikzset{
 ...
 \def\enonce{  \frametitle{Q2 Juillet 2002.}
    ...
    \frame{ 
	  % résolution ex1
		...
		\underline{Résolution}\\ \flushleft
                $\mathcal{P_L} \equiv y = a(x-x_s)^2 + y_s.\ (a\neq 0)$
                   \bigskip
		
		$\begin{cases}P_x=1,\\ P\in\mathcal{P},\\\mathcal{P} \equiv y=x^2.\end{cases}\rightarrow P=(1,1)$. \medskip
		
		\heart Deux droites sont $\bot$ si le produit de leur coefficient angulaire vaut $-1$. \\
		
		\begin{align*} \mathcal{P_L}'(1)\mathcal{P}'(1) =& -1, \\ \qquad 2\, a(1-x_s)\, 2=&-1, \\
a =& \dfrac{1}{4(x_s-1)}. \quad (x_s \neq 1)                \end{align*} \\ \bigskip
		
		
                \begin{align*}P\in \mathcal{P_L} \ : \mathcal{P_L}(1) =& 1.\\
                a(1-x_s)^2 + y_s =& 1. 
                \end{align*}
                
                \onslide<+->\smallskip \ $\rightarrow y_s = -\dfrac{x_s}{4} + \dfrac{5}{4}. \quad (x_s \neq 1)\hspace{2mm} \qed$
	...
	.3 L'équation du lieu est une droite dépourvue de $P$..
	  ...
	  \underline{Résumé}\\ \flushleft
          \begin{itemize}
          \item Deux droites sont $\bot$ si le produit de leur coefficient angulaire vaut $-1$
            \item Avant d'éliminer un paramètre, noter les conditions qu'il impose sur les variables.
          \end{itemize}
    }
